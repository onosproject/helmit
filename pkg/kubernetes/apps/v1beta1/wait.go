...
func (d *Deployment) Wait(ctx context.Context, timeout time.Duration) error {
	return wait.Poll(time.Second, timeout, func() (bool, error) {
		deployment, err := d.Clientset().AppsV1beta1().Deployments(d.Namespace).Get(ctx, d.Name, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
...
func (s *StatefulSet) Wait(ctx context.Context, timeout time.Duration) error {
	return wait.Poll(time.Second, timeout, func() (bool, error) {
		set, err := s.Clientset().AppsV1beta1().StatefulSets(s.Namespace).Get(ctx, s.Name, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		if set.Spec.UpdateStrategy.Type != appsv1beta1.RollingUpdateStatefulSetStrategyType {
			return true, nil
		}
